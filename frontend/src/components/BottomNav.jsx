...
export default function BottomNav() {
  const navItems = [
    ...
    { name: 'Query', path: '/tell-problem', icon: HelpCircle },
    { name: 'Alerts', path: '/live-alerts', icon: Bell },
    { name: 'Settings', path: '/settings', icon: Settings },
  ];

  return (
    <nav className="operations-card rounded-none fixed bottom-0 left-0 right-0 md:hidden border-t border-white/5 z-40 flex justify-around py-1 pb-2 shadow-2xl">
      {navItems.map((item) => (
        <NavLink
          key={item.name}
          to={item.path}
