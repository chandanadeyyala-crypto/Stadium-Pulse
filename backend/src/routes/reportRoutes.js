...
router.post('/fan', verifyAuthToken, async (req, res) => {
  ...
  if (!text || !category || !urgency || !location) {
    return res.status(400).json({
      error: 'Invalid Input',
      message: 'Missing required parameters: text, category, urgency, and location.'
    });
  }
  ...
  const newIncident = {
    id: `fan_inc_${Date.now()}`,
    category,
    severity: severityMap[urgency] || 'warning',
    location,
    originalText: text,
    reportedBy: username,
    status: 'pending_approval',
  ...
  newIncident.englishSummary = `Fan Complaint: ${text}`;
  newIncident.draftFanAlert = `Fan reported ${category} issue at ${location}: "${text}".`;
  ...
  try {
    ...
    const pendingAlert = {
      ...
      message: newIncident.draftFanAlert,
      target: location,
      timestamp: newIncident.timestamp,
      approved: false
    };
    ...
    console.error('Error submitting fan complaint:', error.message);
    res.status(500).json({ error: 'Internal Server Error', message: error.message });
  }
});
