...
// POST /api/alerts - Create and publish an alert immediately (Organizer only)
router.post('/', verifyAuthToken, requireRole(['organizer']), async (req, res) => {
  const { type, severity, message, target } = req.body;
  if (!type || !severity || !message || !target) {
    return res.status(400).json({
      error: 'Invalid Input',
      message: 'Missing required parameters: type, severity, message, and target.'
    });
  }
  ...
  try {
    await venueDataService.addAlert(newAlert);
    res.json({
      success: true,
      alert: newAlert
    });
  } catch (error) {
    console.error('Error broadcasting organizer alert:', error.message);
    res.status(500).json({ error: 'Internal Server Error', message: error.message });
  }
});
